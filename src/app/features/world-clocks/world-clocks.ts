...
export class WorldClocksComponent {
 ...
 readonly worldClockStatuses=computed<WorldClockStatus[]>(()=>this.allPresets.map(loc=>{
  const date=this.activeDate(), sun=calculateSolarPosition(date,loc.latitude,loc.longitude), events=calculateSolarEvents(date,loc.latitude,loc.longitude);
  let localTime='--:--', localTimeWithSeconds='--:--:--', utcOffset='UTC';
  try {
  ...
  else if(altitude>0){astroState='day';statusLabel='Golden Hour';}
  else if(altitude>-6){astroState='civil-twilight';statusLabel='Civil Twilight';}
  else if(altitude>-12){astroState='nautical-twilight';statusLabel='Nautical Twilight';}
  else if(altitude>-18){astroState='astro-twilight';statusLabel='Astronomical Twilight';}
  const [hh,mm,ss]=localTimeWithSeconds.split(':').map(Number);
  const phase=((hh||0)*3600+(mm||0)*60+(ss||0))/86400;
  return {location:loc,localTime,localTimeWithSeconds,solarAltitude:Math.round(altitude*10)/10,astroState,statusLabel,utcOffset,
   sunriseTime:events.sunrise?this.formatTime(events.sunrise,loc.timezone):'--:--',sunsetTime:events.sunset?this.formatTime(events.sunset,loc.timezone):'--:--',
   solarNoonTime:events.solarNoon?this.formatTime(events.solarNoon,loc.timezone):'--:--',dayLengthMinutes:Math.round(events.dayLengthMinutes),
   twoPiPhaseRad:Math.round(phase*2*Math.PI*100)/100,twoPiPhasePercent:Math.round(phase*100)};
 }));
 readonly displayedClocks=computed(()=>{const selected=this.selectedLocation(), statuses=this.worldClockStatuses();
  const major=statuses.filter(s=>(s.location.tier??2)<=2), selectedStatus=statuses.find(s=>s.location.id===selected.id);
  const rest=major.filter(s=>s.location.id!==selected.id); return selectedStatus?[selectedStatus,...rest].slice(0,24):major.slice(0,24);
 });
