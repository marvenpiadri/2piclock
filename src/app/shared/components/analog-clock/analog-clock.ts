...
@Component({
  ...
  template: `
    <div class="relative w-full aspect-square max-w-[190px] mx-auto rounded-full bg-slate-950 border-2 border-emerald-500/40 shadow-xl flex items-center justify-center p-2">
      <svg class="w-full h-full overflow-visible" viewBox="0 0 200 200">
        <!-- Outer Ring Glow -->
        <circle cx="100" cy="100" r="94" fill="none" stroke="rgba(16, 185, 129, 0.25)" stroke-width="2" />
        ...
        @for (tick of minuteTicks; track tick.angle) {
          <line
            ...
            [attr.x2]="tick.x2"
            [attr.y2]="tick.y2"
            [attr.stroke]="tick.isMajor ? 'rgba(255,255,255,0.7)' : 'rgba(148, 163, 184, 0.35)'"
